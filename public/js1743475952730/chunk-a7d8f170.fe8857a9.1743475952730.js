(window["webpackJsonp"] = window["webpackJsonp"] || []).push([
    ["chunk-a7d8f170"], {
        "03b3": function(A, t) {
            A.exports = "data:image/png;base64,"
        },
        "0c4d": function(A, t, i) {},
        "146c": function(A, t) {
            A.exports = "data:image/png;base64,"
        },
        1502: function(A, t) {
            A.exports = "data:image/png;base64,"
        },
        "27d3": function(A, t) {
            A.exports = "data:image/png;base64,"
        },
        6968: function(A, t, i) {
            "use strict";
            i.r(t);
            var e = function() {
                    var A = this,
                        t = A._self._c;
                    return t("div", {
                        staticClass: "head",
                        class: {
                            head_isModule: A.isModule
                        }
                    }, [t("div", {
                        staticClass: "safe"
                    }), t("div", {
                        staticClass: "container flex"
                    }, [A.isBack ? t("div", {
                        staticClass: "back",
                        on: {
                            click: function(t) {
                                return A.getBack()
                            }
                        }
                    }, [A.backWhite ? t("img", {
                        attrs: {
                            src: i("27d3"),
                            alt: ""
                        }
                    }) : t("img", {
                        attrs: {
                            src: i("dbc0"),
                            alt: ""
                        }
                    })]) : A._e(), A.isLeft ? t("div", {
                        staticClass: "back",
                        on: {
                            click: function(t) {
                                return A.getBack()
                            }
                        }
                    }, [t("img", {
                        attrs: {
                            src: i("03b3"),
                            alt: ""
                        }
                    })]) : A._e(), A.leftTitle ? t("div", {
                        staticClass: "left_title"
                    }, [A._v(" " + A._s(A.leftTitle) + " ")]) : A._e(), !A.isName || A.isBack || A.leftTitle ? A._e() : t("div", {
                        staticClass: "myName"
                    }, [t("img", {
                        attrs: {
                            src: i("dd88"),
                            alt: ""
                        }
                    })]), A.isName && A.isBack ? t("div", {
                        staticClass: "name tac"
                    }, [A._v(" " + A._s(A.title) + " ")]) : A._e(), A.title2 ? t("div", {
                        staticClass: "name tac title2"
                    }, [A._v(" " + A._s(A.title2) + " ")]) : A._e(), A.isRight ? t("div", {
                        staticClass: "name1",
                        on: {
                            click: function(t) {
                                return A.path_url("/wallet/detail?id=1")
                            }
                        }
                    }, [t("img", {
                        attrs: {
                            src: "data:image/png;base64,",
                            alt: ""
                        }
                    })]) : A._e(), A.isRule ? t("div", {
                        staticClass: "name2",
                        on: {
                            click: function(t) {
                                return A.path_url("/article?id=TASK_RULES")
                            }
                        }
                    }, [A._v(" " + A._s(A.$t("page5.doc43")) + " ")]) : A._e(), t("div", {
                        staticClass: "flex1"
                    }), A.is_clear ? t("div", {
                        staticClass: "clear",
                        on: {
                            click: A.handleClear
                        }
                    }, [t("img", {
                        attrs: {
                            src: i("146c")
                        }
                    })]) : A._e(), A.isNotices ? t("div", {
                        staticClass: "notice",
                        staticStyle: {
                            "margin-left": "0rem",
                            "margin-right": "0.32rem"
                        },
                        on: {
                            click: A.getNotice
                        }
                    }, [t("img", {
                        attrs: {
                            src: i("cf5c"),
                            alt: ""
                        }
                    }), 0 != A.$store.state.userData.unreadCount && "" != A.$store.state.token ? t("span", [A._v(" " + A._s(A.$store.state.userData.unreadCount > 100 ? "99+" : A.$store.state.userData.unreadCount) + " ")]) : A._e()]) : A._e(), A.isLang ? t("div", {
                        staticClass: "lang",
                        on: {
                            click: A.getLang
                        }
                    }, [t("img", {
                        attrs: {
                            src: i("1502"),
                            alt: ""
                        }
                    })]) : A._e(), A.isKf ? t("div", {
                        staticClass: "notice",
                        on: {
                            click: function(t) {
                                return A.service()
                            }
                        }
                    }, [t("img", {
                        attrs: {
                            src: i("6c04"),
                            alt: ""
                        }
                    })]) : A._e(), A.isNotice ? t("div", {
                        staticClass: "notice",
                        on: {
                            click: A.getNotice
                        }
                    }, [t("img", {
                        attrs: {
                            src: i("cf5c"),
                            alt: ""
                        }
                    }), 0 != A.$store.state.userData.unreadCount ? t("span", [A._v(" " + A._s(A.$store.state.userData.unreadCount > 100 ? "99+" : A.$store.state.userData.unreadCount) + " ")]) : A._e()]) : A._e(), t("div", {
                        staticClass: "head_right"
                    }, [A._t("head_right")], 2)]), A.is_sub ? t("div", [t("van-popup", {
                        style: {
                            height: "100%",
                            width: "80%",
                            background: "#ffffff"
                        },
                        attrs: {
                            position: "left"
                        },
                        model: {
                            value: A.show,
                            callback: function(t) {
                                A.show = t
                            },
                            expression: "show"
                        }
                    }, [t("div", {
                        staticClass: "container"
                    }, [t("div", {
                        staticClass: "close",
                        on: {
                            click: function(t) {
                                A.show = !1
                            }
                        }
                    }, [t("img", {
                        attrs: {
                            src: i("6d74"),
                            alt: ""
                        }
                    })])]), t("my-index")], 1)], 1) : A._e()])
                },
                s = [],
                c = (i("14d9"), i("a573"), {
                    watch: {
                        userData(A, t) {
                            this.show = !1
                        }
                    },
                    computed: {
                        userData() {
                            return this.$store.state.userData.isLogin
                        }
                    },
                    data() {
                        return {
                            show: !1
                        }
                    },
                    name: "titleCenter",
                    props: {
                        isModule: {
                            default: !1
                        },
                        title2: {
                            default: !1
                        },
                        leftTitle: {
                            default: ""
                        },
                        isLeft: {
                            default: !1
                        },
                        isKf: {
                            default: !1
                        },
                        is_sub: {
                            default: !0
                        },
                        isName: {
                            default: !1
                        },
                        isMy: {
                            default: !0
                        },
                        title1: {
                            default: "Quantify"
                        },
                        title: {
                            default: ""
                        },
                        isRight: {
                            default: !1
                        },
                        isRule: {
                            default: !1
                        },
                        isNotice: {
                            default: !0
                        },
                        isNotices: {
                            default: !1
                        },
                        isBack: {
                            default: !1
                        },
                        backWhite: {
                            default: !1
                        },
                        isLang: {
                            default: !0
                        },
                        is_clear: {
                            type: Boolean,
                            default: !1
                        }
                    },
                    methods: {
                        handleClear() {
                            window.location.reload()
                        },
                        service() {
                            this.$post(this.URL.home.config_group, {}).then(A => {
                                if (0 == A.code) {
                                    let t = A.data;
                                    t.map(A => {
                                        "CUSTOMER_SERVICE_LINK" == A.tag && A.url && (window.location.href = A.url)
                                    })
                                }
                            })
                        },
                        path_url(A) {
                            this.$router.push({
                                path: A,
                                query: {}
                            })
                        },
                        go_subnav() {
                            this.show = !0
                        },
                        getLang() {
                            this.$router.push("/my/lang")
                        },
                        getBack() {
                            this.$router.go(-1)
                        },
                        getNotice() {
                            this.$router.push("/notice/index")
                        }
                    }
                }),
                a = c,
                g = (i("f596"), i("2877")),
                n = Object(g["a"])(a, e, s, !1, null, "b73557e2", null);
            t["default"] = n.exports
        },
        "6c04": function(A, t) {
            A.exports = "data:image/png;base64,"
        },
        "6d74": function(A, t) {
            A.exports = "data:image/png;base64,"
        },
        cf5c: function(A, t) {
            A.exports = "data:image/png;base64,"
        },
        dbc0: function(A, t) {
            A.exports = "data:image/png;base64,"
        },
        dd88: function(A, t) {
            A.exports = "data:image/png;base64,"
        },
        f596: function(A, t, i) {
            "use strict";
            i("0c4d")
        }
    }
]);